...
class UserController extends Controller
{
    ...
    public function index()
    {
        ...
        if (!$user) {
            return abort(403, 'Anda harus login untuk mengakses halaman ini');
        }

        $email = $user->email;
        $thisKursus = Kursus::where('email', $email)->first();

        // Cek apakah data kursus ditemukan
        if (!$thisKursus) {
            $kursus = 'Belum terdaftar di kursus manapun';
            $totalDisetujui = 0;
        } else {
            // $kursus = $thisKursus->kursus; kalau ini hanya mengambil satu saja
            $kursus = $thisKursus->pluck('kursus')->implode(', '); // dengan seperti ini dia tdk mengambil 1 saja tapi bisa lebih dari satu
            $totalDisetujui = Kursus::where('email', $email)->where('status', 'disetujui')->count();
        }
